...
class FileController extends AdminController
{
    ...
    protected function grid(): Grid
    {
        return Grid::make(File::query(),
            function (Grid $grid) {
                ...
                $grid->column('bucket_type', admin_trans_field('bucket_type'))->display(function ($bucketType) {
                    return admin_trans_option($bucketType, 'bucket_type');
                });
                $grid->column('upload_path');
                $grid->column('bucket_name');
                $grid->column('bucket_file_path')->width('20%');
            });
    }
    ...
    protected function detail($id): Show
    {
        return Show::make($id, File::query(), function (Show $show) {
            ...
            $show->field('bucket_file_name');
            $show->field('bucket_file_path');
